...
%token <int> T_LNUMBER
%token <string> T_STRING T_VARIABLE TT_CONSTANT_STRING T_INLINE_HTML TT_VARIABLE_VARIABLE TT_SUPERGLOBAL
...
%token T_IF T_ELSE T_ELSEIF T_WHILE T_FOR T_FOREACH T_AS T_SWITCH T_CASE T_DEFAULT T_BREAK T_CONTINUE T_THROW T_TRY T_CATCH T_LIST T_ISSET T_EMPTY T_START_HEREDOC T_END_HEREDOC
...
assignable:
    ...
    | TT_SUPERGLOBAL { Ast.Superglobal $1 }
...
expr:
    ...
    | T_START_HEREDOC double_quoted_content_list T_END_HEREDOC { Ast.ConcatList $2 }
    | TT_MINUS expr %prec UNARY_MINUS { Ast.UnaryMinus $2 }
    | TT_LEFT_PAR expr TT_RIGHT_PAR { $2 }
    | cast expr %prec T_INT_CAST { Ast.Cast ($1, $2) }
    ...
    | T_ISSET TT_LEFT_PAR assignable TT_RIGHT_PAR { Ast.Isset $3 }
    | T_EMPTY TT_LEFT_PAR assignable TT_RIGHT_PAR { Ast.Empty $3 }
